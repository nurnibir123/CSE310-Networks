...
class Sender:
    def __init__(self, base, nextseq, windowSize, estimated_rtt, bufferNext):
        self.base = base
        self.nextseq = nextseq
        self.windowSize = windowSize
        self.estimated_rtt = estimated_rtt
        self.bufferNext = bufferNext
        self.packetBuffer = [Pkt() for i in range(64)]

A_1 = None

class Reciever:
    def __init__(self, expectSeq):
        self.expectSeq = expectSeq
        self.packetToSend = Pkt()
...
def A_output(message):
    ...
    if (A_1.bufferNext - A_1.base) >= 64:
        print("Buffer full. Drop message: ", message.data)
        return
    print("Packet added to buffer")
    somePacket = A_1.packetBuffer[A_1.bufferNext % 64]
    somePacket.seqnum = A_1.bufferNext
    somePacket.payload = message.data
    somePacket.checksum = calculate_checksum(somePacket)
    A_1.bufferNext += 1
    sendWindow()
...
def calculate_checksum(packet):
    checksum = 0
    payload = 0
    for c in packet.payload.decode('UTF-8'):
        payload += ord(c)
    
    checksum = packet.seqnum + packet.acknum + payload
    return checksum

def sendWindow():
    global A_1
    while (A_1.nextseq < A_1.bufferNext) and (A_1.nextseq < A_1.base + A_1.windowSize):
        somePacket = A_1.packetBuffer[A_1.nextseq % 64]
        print("Send packet" + str(somePacket.seqnum) + " " + str(somePacket.payload))
        tolayer3(0, somePacket)
        if A_1.base == A_1.nextseq:
            starttimer(0, A_1.estimated_rtt)
        A_1.nextseq += 1
...
def A_input(packet):
    ...
    if packet.checksum != calculate_checksum(packet):
        ...
        return
    if packet.acknum < A_1.base:
        print("Got NAK " + str(packet.acknum))
    
    print("Got ACK " + str(packet.acknum))
    A_1.base = packet.acknum + 1
    if A_1.base == A_1.nextseq:
        stoptimer(0)
        print("Timer Stopped")
        sendWindow()
        ...
        starttimer(0, A_1.estimated_rtt)
        print("Timer : " + " " + str(A_1.estimated_rtt))
...
def A_timerinterrupt():
    ...
    global A_1
    for i in range(A_1.base, A_1.nextseq):
        somePacket = A_1.packetBuffer[i % 64]
        print("TimerInterrupt: resend packet " + str(somePacket.seqnum))
        tolayer3(0, somePacket)
    starttimer(0, A_1.estimated_rtt)
    print("A_TimerInterrupt: " + str(A_1.estimated_rtt))
...
def A_init():
    print("A_init Called...")
    global A_1 
    A_1 = Sender(base=1, nextseq=1, windowSize=8, estimated_rtt=15, bufferNext=1)
...
def B_input(packet):
    ...
    global B_1
    if packet.checksum != calculate_checksum(packet):
        print("Packet corrupted. Send NAK")
        tolayer3(1, B_1.packetToSend)
        return
    if packet.seqnum != B_1.expectSeq:
        print("Not the expected sequence. Send NAK")
        tolayer3(1, B_1.packetToSend)
        return

    print("B Input: packet recieved ")
    tolayer5(1, packet.payload)

    print("B Input: send ACK")
    B_1.packetToSend.acknum = B_1.expectSeq
    B_1.packetToSend.checksum = calculate_checksum(B_1.packetToSend)
    tolayer3(1, B_1.packetToSend)
...
def B_init():
    print("B_init Called...")
    global B_1 
    B_1 = Reciever(expectSeq=1)
    B_1.packetToSend.seqnum = -1
    B_1.packetToSend.acknum = 0
    B_1.packetToSend.checksum = calculate_checksum(B_1.packetToSend)
